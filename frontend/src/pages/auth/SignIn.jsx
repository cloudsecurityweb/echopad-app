...
function SignIn() {
  ...
  useEffect(() => {
    if (isAuthenticated && !isLoading && !redirectUri && !isLoadingRole) {
      const webRedirectParam = searchParams.get('redirect');
      if (webRedirectParam) {
        window.location.href = webRedirectParam;
      } else if (currentRole === ROLES.CLIENT_ADMIN || currentRole === ROLES.USER_ADMIN) {
        navigate('/');
      } else {
        navigate('/dashboard');
      }
    }
  }, [isAuthenticated, isLoading, navigate, redirectUri, isLoadingRole, currentRole, searchParams]);
