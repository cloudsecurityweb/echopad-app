...
function DashboardSideNav({ isOpen, onClose, isCollapsed, onToggleCollapse }) {
  ...
  const allMenuItems = [
    ...
    },
    {
      ...
      roles: [ROLES.SUPER_ADMIN, ROLES.CLIENT_ADMIN],
    },
    {
      path: '/dashboard/help',
      label: 'Help Center',
  ...
  const isActive = (path) => {
    ...
    if (path === '/dashboard') {
      const roleSpecificDashboards = ['/dashboard/super-admin', '/dashboard/client-admin', '/dashboard/user-admin'];
  ...
  return (
    <>
      {/* Side Navigation */}
      <aside
        className={`fixed top-16 left-0 h-[calc(100vh-4rem)] bg-white shadow-xl z-40 transform transition-all duration-300 ease-in-out ${
            ...
            {menuItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 focus:outline-none ${
