...
import { deleteUser } from '../../utils/user-deletion-service';
...
function DeleteUserModal({ isOpen, onClose, user, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!confirmationEmail.trim()) {
      setErrors({ confirmationEmail: 'Please enter the user\'s email to confirm' });
      return;
    }

    // Validate that confirmation email matches user email
    if (confirmationEmail.trim().toLowerCase() !== user?.email.toLowerCase()) {
      setErrors({ confirmationEmail: 'Email does not match. Please type the user\'s email exactly.' });
      return;
    }
    ...
    try {
      await deleteUser(user.id, getAccessToken);
      showNotification(`User ${user.name} has been permanently removed`, 'success');
    ...
    >
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-semibold text-red-600">Remove User</h2>
          <button
            onClick={onClose}
            disabled={isLoading}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="space-y-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700">
                    This will permanently delete the user and immediately revoke all access. All user data will be removed from the system.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <label htmlFor="confirmationEmail" className="block text-sm font-medium text-gray-700 mb-2">
                Type <span className="font-semibold text-gray-900">{user.email}</span> to confirm
              </label>
              <input
                type="email"
                ...
                onChange={(e) => {
                  ...
                  if (errors.confirmationEmail) {
                    setErrors({ ...errors, confirmationEmail: null });
                  }
                  if (errors.submit) {
                ...
                className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-colors ${
                  errors.confirmationEmail ? 'border-red-300 bg-red-50' : 'border-gray-300'
                }`}
                disabled={isLoading}
                autoFocus
              />
              {errors.confirmationEmail && (
                <p className="mt-1 text-sm text-red-600">{errors.confirmationEmail}</p>
              )}
            </div>
          ...
          <div className="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={isLoading || confirmationEmail.trim().toLowerCase() !== user.email.toLowerCase()}
