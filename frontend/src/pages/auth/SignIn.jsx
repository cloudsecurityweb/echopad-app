...
function SignIn() {
  ...
  useEffect(() => {
    if (isAuthenticated && !isLoading && !redirectUri) {
      navigate('/dashboard');
    }
  }, [isAuthenticated, isLoading, navigate, redirectUri]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        ...
        if (userProfile) {
          // Redirect to dashboard - let Dashboard component handle role-based routing
          navigate('/dashboard');
        }
      } catch (error) {
